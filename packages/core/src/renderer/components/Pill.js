import { BaseComponent, Margin } from "./BaseComponent.js";

const CLOSE_SVG_PATH = "M512 64.303538c-247.25636 0-447.696462 200.440102-447.696462 447.696462 0 247.254314 200.440102 447.696462 447.696462 447.696462s447.696462-200.440102 447.696462-447.696462S759.25636 64.303538 512 64.303538zM710.491727 665.266709c12.491499 12.491499 12.489452 32.729425-0.002047 45.220924-6.246261 6.246261-14.429641 9.370415-22.611997 9.370415s-16.363689-3.121084-22.60995-9.366322L512 557.222971 358.730221 710.491727c-6.246261 6.246261-14.429641 9.366322-22.611997 9.366322s-16.365736-3.125177-22.611997-9.370415c-12.491499-12.491499-12.491499-32.729425 0-45.220924l153.268756-153.266709L313.50725 358.730221c-12.491499-12.491499-12.489452-32.729425 0.002047-45.220924s32.729425-12.495592 45.220924-0.004093l153.268756 153.268756 153.268756-153.268756c12.491499-12.491499 32.729425-12.487406 45.220924 0.004093s12.493545 32.729425 0.002047 45.220924L557.225017 512 710.491727 665.266709z";

export class Pill extends BaseComponent {
  constructor({
    label = "",
    value = "",
    onRemove = null,
    margin = {
      marginTop: Margin.NONE,
      marginRight: Margin.NONE,
      marginBottom: Margin.NONE,
      marginLeft: Margin.NONE,
    },
  }) {
    super({ margin });
    this._label = label;
    this._value = value;
    this._onRemove = onRemove;
  }

  _initVNode() {
    const { h } = Vue;

    const closeBtn = h("div", {
      class: "bl-pill--close",
      onClick: (e) => {
        e.stopPropagation();
        this._onRemove?.(this._value);
      },
    }, [
      h("svg", { viewBox: "0 0 1024 1024", width: "14", height: "14" }, [
        h("path", { d: CLOSE_SVG_PATH }),
      ]),
    ]);

    return h("div", { class: "bl-pill" }, [
      h("div", { class: "bl-pill--text" }, this._label || this._value),
      this._onRemove && closeBtn,
    ]);
  }
}
